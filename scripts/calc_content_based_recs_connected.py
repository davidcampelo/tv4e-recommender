...
class ContentBasedRecommendations(object):
    ...
    def __load_data(self):
        ...
        self.__dataframe_videos['video_contents'] = self.__dataframe_videos[['video_title', 'video_desc']].apply(lambda x: " ".join(x), axis=1)
    ...
    def __calculate_user_profile(self, user_id):
        ...
        for i in range(len(user_profile)):
            for idx, row in self.__dataframe_ratings.iterrows():
                print('i = %s rating = %s content_id = %s' % (i, row.overall_rating_value, row.video_id))
                tokens = self.__tfidf_tokens_dict[row.video_id]
                print('tokens = %s' % tokens)
                user_profile[i] += row.overall_rating_value * tokens[i]
    ...
    def __calculate_recommendations(self):
        ...
        for user_id in users_id:
            ...
            for content_id, token_weights in self.__tfidf_tokens_dict.iteritems(): 
                if content_id not in self.__dataframe_ratings.video_id.values: # not calculating for contents already consumed 
